...
SELECT
 ...
 WHERE D.DEPARTMENT_NAME = (SELECT
                              ...
                              JOIN TB_STUDENT S2 ON(D2.DEPARTMENT_NO = S2.DEPARTMENT_NO)
...
SELECT
 ...
 WHERE CATEGORY = (SELECT
                          CATEGORY
                     FROM TB_DEPARTMENT
                    WHERE DEPARTMENT_NAME = '섬유공학과')
 ORDER BY 1;

-- 공학 계열에 해당하는 전공필수 과목 평점 조회
SELECT
       D.DEPARTMENT_NAME
     , ROUND(AVG(G.POINT), 2)
  FROM TB_DEPARTMENT D
...
--  JOIN TB_GRADE G ON(S.STUDENT_NO = G.STUDENT_NO)
--  JOIN TB_DEPARTMENT D ON(S.DEPARTMENT_NO = D.DEPARTMENT_NO)
-- WHERE D.DEPARTMENT_NAME = '원자력공학과'
-- GROUP BY S.STUDENT_NO, S.STUDENT_NAME, D.DEPARTMENT_NAME
-- ORDER BY 4 DESC;
...
-- 평점이 높은 학생 구하기
  WITH
    ...
    AS (SELECT
          ...
          JOIN TB_GRADE G ON(S.STUDENT_NO = G.STUDENT_NO)
          JOIN TB_DEPARTMENT D ON(S.DEPARTMENT_NO = D.DEPARTMENT_NO)
         WHERE D.DEPARTMENT_NAME = '기계공학과' -- 또는 원자력공학과
         GROUP BY S.STUDENT_NO, S.STUDENT_NAME, D.DEPARTMENT_NAME
         ORDER BY 4 DESC)
SELECT
       G.DEPARTMENT_NAME 학과
     , G.STUDENT_NAME 이름
  FROM TOPN_GRADE G
 WHERE ROWNUM <= 3;
