...
--    (단, 헤더는 "학번", "이름", "입학 년도"가 표시되도록 한다.)
SELECT
       STUDENT_NO 학번
     , STUDENT_NAME 이름
     , TO_CHAR(ENTRANCE_DATE, 'RRRR-MM-DD') "입학 년도"
  FROM TB_STUDENT
 WHERE DEPARTMENT_NO = '002'
 ORDER BY ENTRANCE_DATE;

-- 2. 춘 기술대학교의 교수 중 이름이 세 글자가 아닌 교수가 한 명 있다고 한다.
--    그 교수의 이름과 주민 번호를 화면에 출력하는 SQL 문장을 작성해 보자.
...
--    (단, 교수 중 2000년 이후 출생자는 없으며 출력 헤더는 "교수 이름", "나이"로 한다.
--     나이는 '만'으로 계산한다.)
SELECT
       PROFESSOR_NAME "교수 이름"
     , EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM(TO_DATE('19' || SUBSTR(PROFESSOR_SSN, 1, 2), 'RR'))) + 1 "나이(한국식)"
     , FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE('19' || SUBSTR(PROFESSOR_SSN, 1, 2), 'RR')) / 12) "나이(만)"
     , PROFESSOR_SSN
  FROM TB_PROFESSOR
...
SELECT
       TO_CHAR(TO_DATE('20201225', 'YYMMDD'), 'DAY') 금요일
     , TO_CHAR(TO_DATE('20201225', 'YYMMDD'), 'DY') 금
     , TO_CHAR(TO_DATE('20201225', 'YYMMDD'), 'D') "6"
  FROM DUAL;
...
--    각각 몇 년 몇 월 며칠을 의미할까?
SELECT
       TO_CHAR(TO_DATE('99/10/11','YY/MM/DD'), 'YYYYMMDD') -- 2099년 10월 11일
     , TO_CHAR(TO_DATE('49/10/11','YY/MM/DD'), 'YYYYMMDD') -- 2049년 10월 11일
     , TO_CHAR(TO_DATE('99/10/11','RR/MM/DD'), 'RRRRMMDD') -- 1999년 10월 11일
     , TO_CHAR(TO_DATE('49/10/11','RR/MM/DD'), 'RRRRMMDD') -- 2049년 10월 11일
  FROM DUAL;
...
-- 10. 학과별 학생수를 구하여 "학과 번호", "학생 수(명)" 의 형태로 헤더를 만들어
--     결과 값이 출력되도록 하시오.
SELECT
       DEPARTMENT_NO "학과 번호"
     , COUNT(*) "학생 수(명)"
...
SELECT
  ...
  FROM TB_GRADE
 WHERE STUDENT_NO = 'A112113'
 GROUP BY ROLLUP(SUBSTR(TERM_NO, 1, 4), SUBSTR(TERM_NO, 5, 2)) -- 년도 별 누적 평점 & 총 평점
